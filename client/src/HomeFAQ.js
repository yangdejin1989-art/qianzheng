...
function HomeFAQ({ onViewMore }) {
  ...
  // 响应式：小屏显示2行，桌面显示3�?  const [isMobile, setIsMobile] = useState(typeof window !== 'undefined' ? window.innerWidth < 768 : false);
  // 预览弹窗
  const [previewFaq, setPreviewFaq] = useState(null);

  useEffect(() => {
    // 监听窗口尺寸，决定截断行�?    const onResize = () => setIsMobile(window.innerWidth < 768);
    ...
    const fetchFaqs = async () => {
      try {
        ...
        // 只显示可见的FAQ，按排序字段排序，取�?�?        const visibleFaqs = response.data
          .filter(faq => faq.visible)
          .sort((a, b) => a.order - b.order)
          .slice(0, 4); // 只取�?�?        setFaqs(visibleFaqs);
      } catch (error) {
