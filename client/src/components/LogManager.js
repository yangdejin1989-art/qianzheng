...
const LogManager = ({ token }) => {
  ...
  const cleanOldLogs = async (days = 30) => {
    if (!window.confirm(`确定要清理${days}天前的日志文件吗？`)) return;
    ...
    <div className="log-manager">
      <div className="log-header">
        ...
        <div className="log-actions">
          ...
          <button onClick={() => cleanOldLogs(30)}>🗑️ 清理30天前日志</button>
          <button onClick={() => cleanOldLogs(7)}>🗑️ 清理7天前日志</button>
        </div>
      </div>
        ...
        <div className="log-display">
          {loading ? (
            ...
            <div className="no-logs">
              {selectedFile ? '没有找到日志内容' : '请选择一个日志文件'}
            </div>
          )}
                 </div>
       </div>
